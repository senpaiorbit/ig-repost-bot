...
async def _sdk_download(cl, pk, dest, tag: str):
    dest = Path(dest)
...
class IGClient:
    ...
    async def _pub_download(self, pk, dest_dir) -> Path:
        ...
        path, last_err = await _sdk_download(pub, pk, dest_dir, "public ")
        if path is not None:
    ...
    async def download_video(self, target_url: str, dest_dir) -> Path:
        ...
        path, last_err = await _sdk_download(self.cl, pk, dest, "")
        if path is not None:
            return path
        try:
    ...
    def _cand_url(item) -> Optional[str]:
        try:
            ...
            pass
        try:
            if IGClient._is_self(item):
                return None
        except Exception:
            pass
        if isinstance(item, dict):
            if not item.get("code") and isinstance(item.get("user"), dict):
                return None
        code = None
        mtype = ""
        ptype = ""
    ...
    @staticmethod
    def _is_self(item) -> bool:
        try:
            me = (getattr(settings, "IG_USERNAME", "") or "").strip().lstrip("@").lower()
        except Exception:
            me = ""
        if not me:
            return False
        try:
            if isinstance(item, dict):
                u = item.get("user")
                author = u.get("username") if isinstance(u, dict) else None
            else:
                u = getattr(item, "user", None)
                author = getattr(u, "username", None) if u is not None else None
        except Exception:
            return False
        try:
            return bool(author) and str(author).strip().lower() == me
        except Exception:
            return False
    ...
    def _feed_items(payload) -> list:
        ...
        for entry in raw:
            try:
                out.append(IGClient._unwrap(entry))
            except Exception:
                out.append(entry)
        return out
    ...
    def _unwrap(entry):
        try:
            if isinstance(entry, dict):
                inner = entry.get("media_or_ad")
                if isinstance(inner, dict):
                    return inner
                inner = entry.get("media")
                if isinstance(inner, dict):
                    return inner
                return entry
            inner = getattr(entry, "media_or_ad", None)
    ...
    async def fetch_feed_candidates(self, limit: int = 20) -> list[str]:
        ...
        def _feed_log():
            try:
                print(f"[ig] feed: {len(out)} candidates, {len(self._video_urls)} with direct url, raw={raw} skipped_ad={skipped_ad} skipped_nocode={skipped_nocode}, sample={[str(s)[:60] for s in out[:3]]}")
            except Exception:
                pass
        def _add(item) -> bool:
            nonlocal raw, skipped_ad, skipped_nocode
            raw += 1
            try:
        ...
        if not out:
            try:
                sources = [s.strip().lstrip("@") for s in str(getattr(settings, "SOURCE_USERNAMES", "") or "").split(",")]
            ...
            for u in sources:
                ...
                try:
                    uid = None
                    for id_name in ("user_id_from_username", "user_id_from_name"):
                        id_fn = getattr(self.cl, id_name, None)
                        if id_fn is None:
                            continue
                        try:
                            uid = await _maybe_thread(id_fn, u)
                            break
                        except Exception as e:
                            print(f"[ig] source @{u}: {id_name} failed: {e}")
                            continue
                    if uid is None:
                        print(f"[ig] source @{u}: resolve failed")
                        continue
                    medias = []
                    for m_name in ("user_medias", "user_clips", "get_user_medias", "user_feed"):
                        m_fn = getattr(self.cl, m_name, None)
                        if m_fn is None:
                            continue
                        try:
                            try:
                                medias = await _maybe_thread(m_fn, uid, amount=lim)
                            except TypeError:
                                try:
                                    medias = await _maybe_thread(m_fn, uid, lim)
                                except TypeError:
                                    medias = await _maybe_thread(m_fn, uid)
                            break
                        except Exception as e:
                            print(f"[ig] source @{u}: {m_name} failed: {e}")
                            continue
                    try:
                        items = self._feed_items(medias)
                    except Exception:
                        items = []
                    before = len(out)
                    for item in items:
                        ...
                        if len(out) >= lim:
                            break
                    print(f"[ig] source @{u}: {len(items)} items -> {len(out) - before} candidates")
                except Exception as e:
                    print(f"[ig] source @{u} failed: {e}")
                    continue
        _feed_log()
        return out[:lim]
    ...
    async def download_cover(self, url: Optional[str] = None) -> Path:
        if url is None:
            src = IGClient.default_cover_url()
        elif url:
            src = url
        else:
            try:
                src = settings.COVER_IMAGE_URL or ""
            except Exception:
                src = ""
        async with httpx.AsyncClient(timeout=30, follow_redirects=True) as client:
    ...
    def default_cover_url() -> str:
        try:
            pool = [s.strip() for s in str(getattr(settings, "COVER_IMAGE_URLS", "") or "").replace("\n", ",").split(",")]
        ...
        if pool:
            try:
                ...
                pass
        try:
            return (settings.COVER_IMAGE_URL or "").strip()
        except Exception:
            return ""
